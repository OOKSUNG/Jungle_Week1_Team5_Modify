...
UPlayer::UPlayer()
{
	OutputDebugStringA("Create Player !!!");
	...
	ball = new UBall(
		BallPosition,
		getColorFromEnum(newColor),
		radius,
		TransformBallColorsToEImage(newColor)
	);
	...
	for (int i = 0; i < 5; i++)
	{
		directer[i] = new UBall(MyPos, getColorFromEnum(Red), 0.01, TransformBallColorsToEImage(Red));
	}
}
...
void UPlayer::Update()
{
	if (InputManager::GetInstance()->GetSTATUS()[SPACE ] && bcanShoot)
	{
		Shoot();
		bcanShoot = false;
		InputManager::GetInstance()->SetState(SPACE, false);
	}
...
void UPlayer::Shoot()
{
	SoundManager::GetInstance()->PlaySoundEffect(ESoundEffect::ESE_Shoot);
	...
	for (int i = 0; i < GameRow; i++) {
		for (int j = 0; j < GameCol; j++) {
			...
			if (dir <= 2 * radius)
			{
				...
				SoundManager::GetInstance()->PlaySoundEffect(ESoundEffect::ESE_Collide);
				...
				if ((r % 2 != 0 && lines % 2 == 0) || (r % 2 == 0 && lines % 2 != 0))
				{
					...
					{
						OutputDebugStringA("!!! Add Ball !!!\n");
						map->addBallandPop(minC, minR, ball->Color, bisDead);
						GenerateNewBall();
						return;
					}
				}
